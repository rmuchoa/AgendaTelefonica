...
\section{Webservices}
	...
	\subsection{Funcionamento}
		...
		WebServices normalmente são implementados usando linguagens de programação projetados para interação com a Web, como Java Servlets ou Application Server Pages (ASPs) que exigem um programa de back-end. Essas implementações de Web Services também são normalmente baseados em WSDL ou representados usando WSDL. Ou seja, ou novos serviços podem ser gerados a partir de WSDL ou serviços existentes podem ser descritos usando WSDL. É provável que ambas as abordagens vai pegar, como desenhar e expor serviços da Web é obrigado a ser um processo iterativo. Nem abordagem por si só vai fornecer a melhor solução em todos os casos.

\begin{figure}[h]
	\begin{center}
			\includegraphics[width=200px]{imagens/WebServiceFuncionamento.gif}
				 \caption{DESCRIÇÃO DA IMAGEM}
			\label{fig:imagens/WebFuncionamento.gif}
	\end{center}
\end{figure}
...
\lstset{ 
  ...
  emphstyle       =[2]\color{dkyellow},
	...
	Para a demostração da implementação de um web services, foi desenvolvida uma aplicação de agenda telefônica com servidor(web services) em linguagem de programação PHP junto ao zend framework e no lado do cliente, uma aplicação android desenvolvida com phonegap e framework javascript jquery mobile.
	...
	Para quem for usar o web services sabia as operações que possam ser realizadas, utilizamos o WSDL para descrever os serviço da API em detalhes. Foi utilizada a classe Zend\_Soap\_AutoDiscover que permite detectar automaticamente e realizar o mapeamento do da classe de serviço.
	
	\lstinputlisting[language=PHP,caption={Criação de Web Services},label={listing:CriacaoWebServices}]{web_services_wsdl.php}
