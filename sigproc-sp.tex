...
\section{HTTP}
%TRADUÇÃO BIZARRA
		O Hypertext Transfer Protocol (HTTP) é um protocolo de nível de aplicação para distribuição, colaboração, sistemas de informação multimida. Ele é
		um protocoloco genérico, e sem proprietário, ele pode ser usado para muitos objetivos além do uso para hipertexto, como nomenclatura de servidores
		e gerenciamento de objetos de sistemas distribuidos, através da extensão de seus métodos de requisição, códigos de erros e cabeçalhos. Uma
		característica do HTTP é a escrita e negociação da representação de dados, habilitando os sistemas a construir independentemente os dados a serem
		transferidos.\cite{HTTP-1.1}
...
\lstset{ 
	...
	Para a criação de um web service, utilizamos a classe Zend_Soap_Server que fornece toda funcionalidade de um web service no protocolo SOAP,
	
	\lstinputlisting[language=PHP,caption={Criação de Web Services}]{start_web_services_server.php}
