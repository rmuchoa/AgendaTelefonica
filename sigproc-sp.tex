...
\lstset{ 
	...
	Para a demostração da implementação de um \emph{Web Service}, foi desenvolvida uma aplicação de agenda telefônica com servidor(\emph{Web Service}) em linguagem de programação PHP junto ao \emph{Zend Framework} e uma aplicação cliente para o sistema \emph{Android} desenvolvida com \emph{PhoneGap} e \emph{framework javascript JQuery Mobile}.
	...
	Para realizar a comunição com \emph{Web Service}, foi criada uma classe \emph{WebService} como na listing \ref{listing:ClasseClienteWebservice}, esta classe trata de realizar uma conexão HTTP e mandar o XML por \emph{POST}.
